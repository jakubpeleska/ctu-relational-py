#!/bin/bash
#SBATCH --job-name=resnet_tabular_aggregate_hyperparams
...
EXPERIMENT_NAME="resnet_tabular_aggregate_hyperparams"
...
python -u experiments/original/tabular_hyperparams.py --ray_address=${ray_address} \
  ...
  --dataset=${dataset} --task=${task} --tabular_model="resnet" --aggregate_neighbors \
  --num_samples=${NUM_SAMPLES} --num_cpus=${SLURM_CPUS_PER_TASK} &> "${log_dir}/run.log"
